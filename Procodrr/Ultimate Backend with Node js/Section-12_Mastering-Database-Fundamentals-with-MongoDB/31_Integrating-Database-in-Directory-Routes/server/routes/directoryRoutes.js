...
router.get("/:id?", async (req, res) => {
  ...
  const directories = await Promise.all(
    directoryData.directories.map((dirId) =>
      directoriesCollection.findOne(
        { _id: dirId.id },
        { $projection: { name: 1 } },
      ),
    ),
  );
  const filteredDirectories = directories.map(({ _id, name }) => ({
    id: _id,
    name,
  }));
  ...
  return res
    .status(200)
    .json({ ...directoryData, files, directories: filteredDirectories });
});
...
router.post("/:parentDirId?", async (req, res, next) => {
  ...
  try {
    ...
    const { insertedId } = await directoriesCollection.insertOne({
      name: dirname,
      parentDirId,
      files: [],
      userId: user._id,
    ...
    directoriesCollection.updateOne(
      { _id: parentDirId },
      {
        $push: { directories: { id: insertedId } },
      },
    );
    // parentDir.directories.insertOne({id: insertedId});
...
router.patch("/:id", async (req, res, next) => {
  ...
  try {
    const dirData = await directoriesCollection.findOne({
      _id: new ObjectId(id),
    });
    if (!dirData)
      return res.status(404).json({ message: "Directory not found!" });
    // Check if the directory belongs to the user
    if (dirData.userId.toString() !== user._id.toString()) {
      return res
        .status(403)
