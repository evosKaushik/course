...
app.post("/auth/google/callback", async (req, res) => {
  ...
  const existingUserSession = sessions.find(({ userId }) => userId === sub);

  if (existingUserSession) {
    if (existingUserSession.length > 3) {
      return res.status(400).json({ error: "Maximum number of sessions reached." });
    }
  }
...
app.post("/logout", async (req, res) => {
  ...
  let newSessions = sessions.filter(({ sessionId }) => sid !== sessionId);

  await writeFile("sessionsDB.json", JSON.stringify(newSessions, null, 2));
  res.clearCookie("sid");
