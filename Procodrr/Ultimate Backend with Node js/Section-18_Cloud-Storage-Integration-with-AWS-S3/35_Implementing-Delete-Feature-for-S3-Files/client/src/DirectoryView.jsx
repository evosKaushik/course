...
function DirectoryView() {
  ...
  async function handleFileSelect(e) {
    ...
    try {
      const { uploadURL, fileId } = await initializeFileUpload(
        dirId,
        tempItem.name,
        tempItem.size
      );
      handleS3FileUpload(file, uploadURL, fileId);
    } catch (err) {
      setErrorMessage(err.response?.data?.error || "Failed to start upload");
  ...
  const handleS3FileUpload = (file, uploadURL, fileId) => {
    ...
    xhr.onload = async () => {
      try {
        ...
        setErrorMessage("Upload verification failed. The file may not have saved.");
        setUploadItem(null);
        setTimeout(() => setErrorMessage(""), 3000);
      }
    };
