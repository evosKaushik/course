...
export const uploadFile = async (req, res, next) => {
  ...
  try {
    ...
    if (filesize > remainingSpace) {
      ...
      return res.status(404).json({ error: "File too large!" });
    }
    ...
    const uploadURL = await generateUploadUrl(fullFileName);
...
export const deleteFile = async (req, res, next) => {
  ...
  try {
    ...
    await deleteFileFromServer(`${id}${file.extension}`);
