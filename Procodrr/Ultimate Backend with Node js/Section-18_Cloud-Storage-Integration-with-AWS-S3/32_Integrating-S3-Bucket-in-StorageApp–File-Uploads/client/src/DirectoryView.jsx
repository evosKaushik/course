...
function DirectoryView() {
  ...
  async function handleFileSelect(e) {
    ...
    // const URL = startUpload(tempItem);
    ...
    const { data } = await axiosWithCreds.post(
      ...
      {
        headers: {
          filename: tempItem.name,
          filesize: tempItem.size.toString(),
        },
      },
    );
    console.log(data);
    ...
    // console.log("URL", URL);
    console.log(file);
    handleServerFileUpload(file, data.uploadURL);
  }

  const handleServerFileUpload = (file, URL) => {
    ...
    xhr.onerror = () => {
      ...
      setFilesList((prev) => prev.filter((f) => f.id !== uploadItem?.id));
      setUploadItem(null);
      setTimeout(() => setErrorMessage(""), 3000);
    };
  ...
  //   xhr.open("POST", `http://localhost:3000/file/${dirId || ""}`);
  ...
  //   xhr.onload = () => {
  //     console.log("Status:", xhr.status); // 200, 201, 400, etc.
  //     console.log("Response:", xhr.responseText); // Response body as text

  //     // If the server returns JSON
  //     try {
  //       const { uploadURL } = JSON.parse(xhr.responseText);
  //       URL = uploadURL;
