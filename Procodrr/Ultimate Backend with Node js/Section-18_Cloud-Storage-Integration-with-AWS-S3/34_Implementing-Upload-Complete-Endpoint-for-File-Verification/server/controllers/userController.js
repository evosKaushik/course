...
export const login = async (req, res, next) => {
  ...
  await redisClient.sAdd(sessionKey, redisKey);
...
export const logout = async (req, res) => {
  ...
  if (raw) {
    ...
    await redisClient.sRem(`user_sessions:${session.userId}`, `session:${sid}`);
  }
...
export const logoutAll = async (req, res) => {
  ...
  const raw = await redisClient.get(`session:${sid}`);
  const session = JSON.parse(raw);
  const sessionKey = `user_sessions:${session.userId}`;
  const allSessionIds = await redisClient.sMembers(sessionKey);
  if (allSessionIds.length > 0) {
    await redisClient.del(allSessionIds);
  }
